  ...
  template: `
    <div class="fade-in space-y-6">
      ...
      <div class="glass-card p-6 relative overflow-hidden">
        <div class="absolute inset-0 bg-gradient-to-r from-blue-600/10 via-transparent to-green-600/5 pointer-events-none"></div>
        <div class="relative">
          <p class="text-muted text-sm mb-1">Available Balance</p>
          <div class="text-4xl font-bold text-white mb-1">₹{{ user()?.walletBalance?.toLocaleString('en-IN') }}</div>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="stat-card">
          ...
          <div class="text-xl font-bold text-white">₹1,500</div>
        </div>
        <div class="stat-card">
          <div class="text-muted text-xs mb-2">Total Withdrawn</div>
          <div class="text-xl font-bold text-white">₹3,000</div>
        </div>
        <div class="stat-card">
          <div class="text-muted text-xs mb-2">Prize Earnings</div>
          <div class="text-xl font-bold text-green-400">₹8,550</div>
        </div>
        <div class="stat-card">
          <div class="text-muted text-xs mb-2">Entry Fees Paid</div>
          <div class="text-xl font-bold text-red-400">₹300</div>
        </div>
      </div>

      <!-- Transaction History -->
      <div class="glass-card overflow-hidden">
        <div class="p-5 border-b border-blue-900/20 flex items-center justify-between">
          <h3 class="text-white font-bold">Transaction History</h3>
          ...
          @for (tx of transactions; track tx.id) {
            <div class="px-5 py-4 flex items-center gap-4 table-row">
              ...
              <div class="flex-1 min-w-0">
                <div class="text-white font-medium text-sm truncate">{{ tx.description }}</div>
                <div class="text-muted text-xs mt-0.5">{{ formatDate(tx.date) }}</div>
              </div>
              <div class="text-right flex-shrink-0">
                <div class="font-bold text-sm" [class]="tx.amount > 0 ? 'text-green-400' : 'text-red-400'">
                  {{ tx.amount > 0 ? '+' : '' }}₹{{ Math.abs(tx.amount).toLocaleString('en-IN') }}
                </div>
                <div class="text-xs mt-0.5"
                  [class]="tx.status === 'success' ? 'text-green-500' : tx.status === 'pending' ? 'text-yellow-500' : 'text-red-500'">
                  {{ tx.status }}
                </div>
              </div>
            </div>
      ...
      @if (showDepositModal()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div class="glass-card p-6 w-full max-w-md">
            <h3 class="text-white font-bold text-lg mb-4">Add Money to Wallet</h3>
            ...
            <div class="mb-4">
              <label class="block text-sm font-medium text-slate-300 mb-2">Custom Amount</label>
              ...
              <div class="space-y-2">
                @for (method of paymentMethods; track method.id) {
                  <label class="flex items-center gap-3 p-3 rounded-xl border cursor-pointer"
                    [class]="selectedMethod === method.id ? 'border-blue-500/50 bg-blue-500/10' : 'border-blue-900/20 hover:border-blue-900/40'">
                    <input type="radio" name="payMethod" [value]="method.id" [(ngModel)]="selectedMethod" class="hidden"/>
                    <div class="w-8 h-8 rounded-lg bg-blue-500/15 flex items-center justify-center text-base">{{ method.icon }}</div>
                    <div>
                      <div class="text-white text-sm font-medium">{{ method.name }}</div>
      ...
      @if (showWithdrawModal()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div class="glass-card p-6 w-full max-w-md">
            <h3 class="text-white font-bold text-lg mb-4">Withdraw Funds</h3>
            <p class="text-muted text-sm mb-4">Available: ₹{{ user()?.walletBalance?.toLocaleString('en-IN') }}</p>

            <div class="mb-4">
              <label class="block text-sm font-medium text-slate-300 mb-2">Withdrawal Amount</label>
              <input type="number" class="input-field" placeholder="Enter amount" [(ngModel)]="withdrawAmount" />
            </div>

            <div class="mb-4">
              <label class="block text-sm font-medium text-slate-300 mb-2">Bank Account / UPI</label>
              <input type="text" class="input-field" placeholder="Enter UPI ID or account number" [(ngModel)]="withdrawAccount" />
            </div>

            <div class="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 mb-5">
              <p class="text-yellow-400 text-xs">⚠️ Withdrawals are processed within 24-48 hours. Minimum withdrawal is ₹100.</p>
            </div>
...
export class WalletComponent implements OnInit {
  ...
  getTxIconClass(type: string): string {
    const classes: Record<string, string> = {
      ...
      prize: 'bg-green-500/15 text-green-400',
      entry_fee: 'bg-red-500/15 text-red-400'
    };
    return classes[type] || 'bg-slate-500/15';
  }

  getTxIcon(type: string): string {
    const icons: Record<string, string> = {
      deposit: '↓', withdrawal: '↑', prize: '🏆', entry_fee: '🎯'
    };
    return icons[type] || '💰';
  }
