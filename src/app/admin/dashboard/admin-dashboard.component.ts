    ...
    <div class="fade-in space-y-6">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="stat-card">
          ...
          <div class="text-2xl font-bold text-slate-800">{{ stats?.totalUsers?.toLocaleString('en-IN') }}</div>
          <div class="text-green-600 text-xs mt-1">+{{ stats?.todaySignups }} today</div>
        </div>
        ...
        <div class="stat-card">
          ...
          <div class="text-green-600 text-xs mt-1">₹{{ stats?.todayRevenue?.toLocaleString('en-IN') }} today</div>
        </div>
        ...
        <div class="stat-card">
          ...
          <div class="text-2xl font-bold text-slate-800">{{ stats?.activePlayers?.toLocaleString('en-IN') }}</div>
            ...
            @for (user of recentUsers; track user.id) {
              <div class="px-5 py-3 flex items-center gap-3 table-row">
                ...
                <div class="flex-1 min-w-0">
                  <div class="text-slate-800 text-sm font-medium truncate">{{ user.name }}</div>
                  <div class="text-muted text-xs truncate">{{ user.email }}</div>
                </div>
                <div class="text-right flex-shrink-0">
                  <div class="text-green-600 text-xs font-semibold">₹{{ user.walletBalance.toLocaleString('en-IN') }}</div>
          ...
          <div class="divide-y divide-slate-100">
            @for (quiz of todayQuizzes; track quiz.id) {
              <div class="px-5 py-3 flex items-center gap-3 table-row">
                ...
                <div class="text-right flex-shrink-0">
                  <div class="text-green-600 text-xs font-semibold">₹{{ quiz.prizePool.toLocaleString('en-IN') }}</div>
                  <div class="mt-0.5">
                    @if (quiz.status === 'live') {
